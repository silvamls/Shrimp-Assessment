...
# Sensitivity analysis to evaluate priors hypothesis
# Combining final results of reconstructed time series (Silva and Freire)
# Created and Modified by Matheus L. (Silvamls) 04/2026
##############################################################################################
...
#library(keras)
#install_keras(method = "virtualenv", python_version = "3.11.3") #now you can install KERAS API in R
#testing 
library(keras)
to_categorical(0:3)
...
# URL from IBGE for Brazil (2020)
url_uf <- "https://geoftp.ibge.gov.br/organizacao_do_territorio/malhas_territoriais/malhas_municipais/municipio_2020/Brasil/BR/BR_UF_2020.zip"

# download and extract
temp_zip <- tempfile(fileext = ".zip")
download.file(url_uf, temp_zip, mode = "wb")
unzip(temp_zip, exdir = "estados_ibge")

# read shapefile (.shp)
br_states <- sf::st_read("estados_ibge/BR_UF_2020.shp")
...
coastal_states <- br_states %>%
  mutate(region = case_when(
    SIGLA_UF %in% c("AC", "AM", "RO", "RR", "TO", "GO", "MT", "DF", "MS", "MG") ~ "Other",
    SIGLA_UF %in% c("AP", "PA", "MA") ~ "North",
    SIGLA_UF %in% c("CE", "RN", "PB", "PE", "AL", "SE", "BA", "PI") ~ "Northeast",
    SIGLA_UF %in% c("ES", "RJ", "SP") ~ "Southeast",
    SIGLA_UF %in% c("PR", "SC", "RS") ~ "South",
    TRUE ~ "Other"
  ))
...
plot_data_all <- bind_rows(plot_data_obs_sep, plot_data_pred_sep) %>%
  ...
  mutate(source = factor(source, levels = c("Silva", "Freire")),
         stock = paste(category,region, sep = "_"))             # Set factor order for source
...
p2 <- ggplot(plot_data_all, 
  ...
  facet_wrap(.~ stock, scales = "free_y") + # Facet by region and category, free y-axis
...
bk_all <- bind_rows(bk_all_list) %>%
  separate(stock,
           ...
           remove = FALSE) %>%
  mutate(
    bk_mean_adj = pmax(bk, 0.1),  # truncation if is <0.1
    bk_lo = round(pmax(bk_mean_adj - 0.1, 0.05), 2), #simetric intervals
    bk_hi = round(pmin(bk_mean_adj + 0.1, 0.9), 2),
    bk = round(bk_mean_adj, 2),
...
# life history summary table
lf_params_summary <- lf %>%
  dplyr::group_by(Especie) %>%
  dplyr::summarise(
    Linf_mean = mean(`Linf(mm)TL`, na.rm = TRUE) / 10,
    k_mean    = mean(`K(ano)`, na.rm = TRUE),
    M_mean    = mean(M, na.rm = TRUE),
    L50_mean  = mean(`L50(mm)TL`, na.rm = TRUE) / 10,
    tmax_mean = mean(`Tmáx`, na.rm = TRUE),
    
    n_studies = dplyr::n(),
    
    sources = paste(unique(Autor), collapse = "; "),
    regions = paste(unique(Regiao), collapse = "; ")
  ) %>%
  dplyr::mutate(across(where(is.numeric), ~ round(., 2)))

write.csv(lf_params_summary, "lf_params_summary.csv", row.names = FALSE)
...
# Using BRT  for B/k prior depletions
# Using Euler-Lotka, Myers.. for intrinsic growth rate (r)
#------------------------------------------------------------

#stocks analyzed 
stocks <- tibble(
  stock = unique(bk_all$stock)
) %>%
  separate(
    stock,
    into = c("category", "region", "source"),
  ...
  mutate(
    init_yr = 1950,
    end_yr = 2015,
    type_data = "reconstructed"
  )

# 1. #List scenarios which will be tested
#Baseline (CMSY-default)~ #CMSY resilience-based r (0.2–0.8, medium resilience); B/k estimated by CMSY neural net
#Informed (Biological + Statistical)	r derived from Euler–Lotka; B/k estimated using BRT; moderately constrained priors
#Economic hypothesis (High B/k)	Same r as the informed scenario; relaxed high B/k prior (e.g. 0.3–0.8)
#Literature hypothesis	Same r as the informed scenario; restrictive B/k prior: low (0.1–0.3) for South/Southeast and medium (0.4–0.6) for North/Northeast
#Uninformative (Wide priors)	Wide priors for both parameters: r = 0.1–0.9; B/k = 0.01–0.9
#Uninformative (Fixed r / Flexible B/k)	r fixed based on Euler–Lotka (life-history estimate); wide B/k prior (0.01–0.9)
#Uninformative (Fixed B/k / Flexible r)	B/k fixed based on BRT estimates; wide r prior (0.1–0.9)
#Forecast (Projected catches via neural network)	Same as the informed scenario (Euler–Lotka–derived r and BRT-based B/k priors)
cenarios <- c(
...
# 2. Expand grid
scenarios_dat <- expand_grid(stocks, scenario = cenarios)

# 3. type_data (only for forecast)
scenarios_dat <- scenarios_dat %>%
  mutate(type_data = if_else(scenario == "Forecast (Projected catches via neural network)", 
                             "projected", type_data))

# Update end_yr when scenario == Projected
scenarios_dat <- scenarios_dat %>%
  mutate(end_yr = if_else(type_data == "projected", 2025, end_yr))

# 4. r_method
scenarios_dat <- scenarios_dat %>%
  mutate(r_method = case_when(
    scenario == "Baseline (CMSY-default)" ~ "Resilience category (CMSY-default)",
    scenario == "Informed (Biological + Statistical)" ~ "Euler-Lotka derived methods",
    scenario == "Economic hypothesis (High B/k)" ~ "Euler-Lotka derived methods",
    scenario == "Literature hypothesis" ~ "Euler-Lotka derived methods",
    scenario == "Uninformative (Wide priors)" ~ "Uninformative (Wide priors)",
    scenario == "Uninformative (Fixed r / Flexible B/k)" ~ "Euler-Lotka derived methods",
    scenario == "Uninformative (Fixed B/k / Flexible r)" ~ "Uninformative (Wide r)",
    scenario == "Forecast (Projected catches via neural network)" ~ "Euler-Lotka derived methods",
    TRUE ~ NA_character_
  ))

# 5. bk_method
scenarios_dat <- scenarios_dat %>%
  mutate(bk_method = case_when(
    scenario == "Baseline (CMSY-default)" ~ "Neural network (CMSY-default)",
    scenario == "Informed (Biological + Statistical)" ~ "Boosted Regression Tree- BRT",
    scenario == "Economic hypothesis (High B/k)" ~ "relaxed high B/k prior",
    scenario == "Literature hypothesis" ~ "Literature informed B/k",
    scenario == "Uninformative (Wide priors)" ~ "Uninformative (Wide priors)",
    scenario == "Uninformative (Fixed r / Flexible B/k)" ~ "Uninformative (Wide B/k)",
    scenario == "Uninformative (Fixed B/k / Flexible r)" ~ "Boosted Regression Tree- BRT",
    scenario == "Forecast (Projected catches via neural network)" ~ "Boosted Regression Tree- BRT",
    TRUE ~ NA_character_
  ))

# 6. r_min e r_max ------------------------------------------------

# Helper function to search for r in the r_summary table by species (may return NA if not found)
get_r <- function(specie, tipo = "min") {
  vals <- r_summary[r_summary$specie == specie,
                    paste0("r_", tipo)]
  if (length(vals) == 0) return(NA_real_)
  # raw value
  out <- min(vals, na.rm = TRUE)
  # apply limit for extreme low r
  if (tipo == "min") {
    out <- max(out, 0.1)
  }
  round(out, 2)
...
# Apply with case_wend (fixed)
scenarios_dat <- scenarios_dat %>%
  mutate(
    r_min = case_when(
      # Baseline
      scenario == "Baseline (CMSY-default)" ~ 0.6, #high resilience
      
      # Informed, Economic, Literature, Forecast (usam Euler-Lotka)
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "brown" ~ get_r("F. subtilis", "min"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "pink" ~ get_r("F. paulensis", "min"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "seabob" ~ get_r("X. kroyeri", "min"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "white" ~ get_r("L. schmitti", "min"),
      
      # Uninformative (Wide priors)
      scenario == "Uninformative (Wide priors)" ~ 0.1,
      
      # Uninformative (Fixed r / Flexible B/k) -> uses Euler-Lotka the same as informed
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "brown" ~ get_r("F. subtilis", "min"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "pink" ~ get_r("F. paulensis", "min"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "seabob" ~ get_r("X. kroyeri", "min"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "white" ~ get_r("L. schmitti", "min"),
      
      # Uninformative (Fixed B/k / Flexible r) -> wide r
      scenario == "Uninformative (Fixed B/k / Flexible r)" ~ 0.1,
      
      TRUE ~ NA_real_
    ),
    
    r_max = case_when(
      scenario == "Baseline (CMSY-default)" ~ 1.5,
      
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "brown" ~ get_r("F. subtilis", "max"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "pink" ~ get_r("F. paulensis", "max"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "seabob" ~ get_r("X. kroyeri", "max"),
      scenario %in% c("Informed (Biological + Statistical)",
                      "Economic hypothesis (High B/k)",
                      "Literature hypothesis",
                      "Forecast (Projected catches via neural network)") & category == "white" ~ get_r("L. schmitti", "max"),
      
      scenario == "Uninformative (Wide priors)" ~ 1.5,
      
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "brown" ~ get_r("F. subtilis", "max"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "pink" ~ get_r("F. paulensis", "max"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "seabob" ~ get_r("X. kroyeri", "max"),
      scenario == "Uninformative (Fixed r / Flexible B/k)" & category == "white" ~ get_r("L. schmitti", "max"),
      
      scenario == "Uninformative (Fixed B/k / Flexible r)" ~ 1.5,
...
# table to extract bk_lo/hi from bk_all
brt_bk <- bk_all %>%
  filter(method == "BRT") %>%
  select(category, region, source, year, bk_lo, bk_hi) %>%
  mutate(
    bk_min_brt = bk_lo,
    bk_max_brt = bk_hi
  ) %>%
...
#Create a year column to reference the b/k for 2015 and 2025.
scenarios_dat <- scenarios_dat %>%
  mutate(ano_ref = case_when(
    scenario == "Forecast (Projected catches via neural network)" ~ 2025,
    TRUE ~ 2015
  ))
#left join according to the reference year
scenarios_dat <- scenarios_dat %>%
  left_join(brt_bk, by = c("category", "region", "source", "ano_ref" = "year"))

#create rules for each b/k scenario.
scenarios_dat <- scenarios_dat %>%
  mutate(
    bk_min = case_when(
      scenario == "Baseline (CMSY-default)" ~ NA_real_,
      scenario == "Informed (Biological + Statistical)" ~ bk_min_brt,
      scenario == "Economic hypothesis (High B/k)" ~ 0.3,
      scenario == "Literature hypothesis" & region %in% c("S", "SE") ~ 0.1,
      scenario == "Literature hypothesis" & region %in% c("N", "NE") ~ 0.35,
      scenario == "Uninformative (Wide priors)" ~ 0.01,
      scenario == "Uninformative (Fixed r / Flexible B/k)" ~ 0.01,
      scenario == "Uninformative (Fixed B/k / Flexible r)" ~ bk_min_brt,
      scenario == "Forecast (Projected catches via neural network)" ~ bk_min_brt,
      TRUE ~ NA_real_
    ),
    bk_max = case_when(
      scenario == "Baseline (CMSY-default)" ~ NA_real_,
      scenario == "Informed (Biological + Statistical)" ~ bk_max_brt,
      scenario == "Economic hypothesis (High B/k)" ~ 0.8,
      scenario == "Literature hypothesis" & region %in% c("S", "SE") ~ 0.4,
      scenario == "Literature hypothesis" & region %in% c("N", "NE") ~ 0.65,
      scenario == "Uninformative (Wide priors)" ~ 0.9,
      scenario == "Uninformative (Fixed r / Flexible B/k)" ~ 0.9,
      scenario == "Uninformative (Fixed B/k / Flexible r)" ~ bk_max_brt,
      scenario == "Forecast (Projected catches via neural network)" ~ bk_max_brt,
      TRUE ~ NA_real_
    )
  ) %>%
  select(-bk_min_brt, -bk_max_brt, -ano_ref)  #remove auxiliary columns
#-------------------------------------------------------------------

# final result
glimpse(scenarios_dat)
