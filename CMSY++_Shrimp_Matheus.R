...
setwd("C:/Matheus/Universidade/Doutorado/Shrimp Stock Assessment- FAO") # set working directory to source file location
...
id_file     <- "cinfo_shrimp_1.csv" #"Stocks_ID_forDeng_RSb - Copy.csv" # "CombStocks_ID_2020CMSYrun3_v3_RF3.csv"   #    "SAUP_ID_2.csv"    #"SimSpecCPUE_4_NA_int_end.csv"  #"Train_ID_7j.csv" # "Stocks_ID_Aust_4.csv"  #"STECF_ID_2020_2.csv" #tRFMO_ID_2020_2.csv" #"ICES_ID_2020_4.csv" #"Robust_Stocks_ID_10_allNA.csv" #"SimSpecCPUE_4.csv"#"Stocks_ID_R_7.csv"  #"Stock_ID_forRainer.csv"  #  "NCod_ID_4.csv" #"SimSpecCPUE_3.csv" #  name of file containing stock-specific info and settings for the analysis
...
out_list    <- list()
bioout<-data.frame(yr=NULL,region=NULL,category=NULL,source=NULL,nyr=NULL,start.yr=NULL,int.yr=NULL,end.yr=NULL,
                   ucl.B.Bmsy=NULL,lcl.B.Bmsy=NULL,B.Bmsy=NULL,
                   ucl.F.Fmsy=NULL,lcl.F.Fmsy=NULL,F.Fmsy=NULL)
pprout=data.frame(region=NULL,category=NULL,source=NULL,priorr=NULL,postr=NULL,PPVRr=NULL,
                  mean.log.r=NULL,sd.log.r=NULL,mean.log.k=NULL,sd.log.k=NULL)
ppbkout=data.frame(region=NULL,category=NULL,source=NULL, priorbk=NULL,postbk=NULL,PPVRbk= NULL )

kobeout=data.frame(region=NULL,category=NULL,source=NULL, x.F_Fmsy=NULL,y.b_bmsy=NULL)
...
#----------------------------------------
#stockpick= "ct_Matheus_SE_white"
#stks<- cinfo$Stock[unique(grep(stockpick,cinfo$Stock))]

stks<- unique(cinfo$Stock)
#stks="ct_Matheus_SE_white"
stks<- "Freire_SE_white"
...
region=strsplit(stk,"_")[[1]][2]
category=strsplit(stk,"_")[[1]][3]
source=strsplit(stk,"_")[[1]][1]
biodat<-data.frame(yr=yr,region=region,category=category,source=source,nyr=nyr,start.yr=start.yr,int.yr=int.yr,end.yr=end.yr,
                   ucl.B.Bmsy=ucl.B.Bmsy,lcl.B.Bmsy=lcl.B.Bmsy,B.Bmsy=B.Bmsy,
                   ucl.F.Fmsy=ucl.F.Fmsy,lcl.F.Fmsy=lcl.F.Fmsy,F.Fmsy=F.Fmsy)
bioout= rbind(bioout,biodat)
#---------------------------

#prior posteriors data
#get priors and posteriors for r
# plot PP-diagnostics for CMSY
# r
rk <- exp(mvn(n=length(rs),mean.log.r=mean.log.r,sd.log.r=sd.log.r,mean.log.k=mean.log.k,sd.log.k=sd.log.k))
priorr = sort(rk[,1])
postr = rs

PPVRr = round((sd(postr)/mean(postr))^2/(sd(priorr)/mean(priorr))^2,2)

pprdat=data.frame(region=region,category=category,source=source,priorr=priorr,postr=postr,PPVRr=PPVRr,
                 mean.log.r=mean.log.r,sd.log.r=sd.log.r,mean.log.k=mean.log.k,sd.log.k=sd.log.k)
pprout= rbind(pprout,pprdat)


#get priors and posteriors for b/k 2015
# bk3
postbk = all.bk.cmsy[,length(yr)]
priorbk <- sort(rbeta(length(postbk),bk.beta[1,3], bk.beta[2,3]))

PPVRbk = round((sd(postbk)/mean(postbk))^2/(sd(priorbk)/mean(priorbk))^2,2)


ppbkdat=data.frame(region=region,category=category,source=source, 
                   priorbk=priorbk,postbk=postbk, PPVRbk= PPVRbk)


ppbkout= rbind(ppbkout,ppbkdat)


#get data to construct Kobe plots compositions
kobedat=data.frame(region=region,category=category,source=source, x.F_Fmsy=x.F_Fmsy,
                   y.b_bmsy=y.b_bmsy)
kobeout= rbind(kobeout,kobedat)


# out_list[[paste(stk)]]<-list(yr=yr,nyr=nyr,ucl.B.Bmsy=ucl.B.Bmsy,lcl.B.Bmsy=lcl.B.Bmsy,B.Bmsy=B.Bmsy,
#                              ucl.F.Fmsy=ucl.F.Fmsy,lcl.F.Fmsy=lcl.F.Fmsy,F.Fmsy=F.Fmsy,mean.log.r=mean.log.r,
#                              sd.log.r=sd.log.r,mean.log.k=mean.log.k,sd.log.k=sd.log.k,rs=rs)



}#end of stocks loop (Matheus)



#catch time series (ct_Matheus, ct_Freire, ct_Average)
p1<- ggplot() +
  geom_line(data=bioout, 
            aes(x=yr, y=B.Bmsy,col='B/Bmsy',linetype=source),linewidth=1.5)+
  geom_line(data=bioout, 
            aes(x=yr, y=F.Fmsy,col="F/Fmsy",linetype=source),linewidth=1.5)+
  geom_hline(yintercept=1, linetype=2)+
  facet_wrap(region~category,scales = "free_y")+
  labs(x="Year",y=expression(F/F[MSY]~B/B[MSY]),col="",fill="",linetype="")+
  scale_y_continuous() +
...
ggsave("Biomass_Exploitation_series.png",plot=p1,device="png",units="cm",width = 30, height = 17)



#PRIOR- POSTERIOR PLOTS  r
# plot PP-diagnostics for CMSY
p2<-ggplot2::ggplot(pprout) + 
    geom_density(alpha=.5,aes(x=priorr,col="Prior r",fill="Prior r",linetype=source),lwd=1.5)+
    geom_density(alpha=.5,aes(x=postr,col="Posterior r",fill="Posterior r",linetype=source),lwd=1.5)+
    facet_wrap(region~category,scales = "free_y")+
    labs(x="Intrinsic growth rate (r)",y="Density",fill="",color="",linetype="")+
...
ggsave("PP_r_Density.png", plot = p2, device = "png", units = "cm", width = 30, height = 17)
  

#PRIOR- POSTERIOR PLOTS b/k 2015
# plot PP-diagnostics for CMSY
p3<-ggplot2::ggplot(ppbkout) + 
  geom_density(alpha=.5,aes(x=priorbk,col="Prior b/k 2015",fill="Prior b/k 2015",linetype=source),lwd=1.5)+
  geom_density(alpha=.5,aes(x=postbk,col="Posterior b/k 2015",fill="Posterior b/k 2015",linetype=source),lwd=1.5)+
  facet_wrap(region~category,scales = "free_y")+
  labs(x="b/k 2015",y="Density",fill="",color="",linetype="")+
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_manual(values = c("gray30",'gray60')) +
  scale_color_manual(values = c("gray30",'gray60')) +
  theme_classic(base_size = 14) %+replace%
...
ggsave("PP_bk2015_Density.png", plot = p3, device = "png", units = "cm", width = 30, height = 17)
...
{png("Kobe.png",width=32,height=19,units="cm",
    res=800,
    antialias="cleartype")
  par(mfrow=c(3,4),mar=c(1.8,2.1,0.1,0),
      oma=c(1.15,2.11,0.5,0.5),xpd=F,
      mgp=c(1.1,0.55,0), cex.lab=0.9,cex.axis=0.7, bty="L",las=1,
      col.axis='gray30',col.lab='gray10')
  

category= unique(kobeout$category)
region= unique(kobeout$region)
for (i in category) {
  for (j in region) {
    
start.yr= unique(bioout$start.yr[bioout$category==i & bioout$region==j])
int.yr= unique(bioout$int.yr[bioout$category==i & bioout$region==j])
end.yr= unique(bioout$end.yr[bioout$category==i & bioout$region==j])
nyr= unique(bioout$nyr[bioout$category==i & bioout$region==j])
B.Bmsy= data.frame(yr=bioout$yr[bioout$category==i & bioout$region==j],
                   b=bioout$B.Bmsy[bioout$category==i & bioout$region==j],
                   source=bioout$source[bioout$category==i & bioout$region==j])
F.Fmsy= data.frame(yr=bioout$yr[bioout$category==i & bioout$region==j],
                   f=bioout$F.Fmsy[bioout$category==i & bioout$region==j],
                   source=bioout$source[bioout$category==i & bioout$region==j])

#f/fmsy and b/bmsy in the last year (2015)
x.F_Fmsy = kobeout$x.F_Fmsy[kobeout$category==i & kobeout$region==j]
y.b_bmsy = kobeout$y.b_bmsy[kobeout$category==i & kobeout$region==j]
#kernel.temp <- ci2d(x.F_Fmsy,y.b_bmsy,nbins=100,factor=10,ci.levels=c(0.50,0.80,0.95),show="none")
#kernelF = kernel.temp
...
#polygon(kernelF$contours$"0.8"[,2:1],border=NA,lty=2,col="grey")
#polygon(kernelF$contours$"0.5"[,2:1],border=NA,lty=2,col="cornsilk2")
points(y.b_bmsy,x.F_Fmsy,pch=20,col="blue")

sources=unique(B.Bmsy$source)
for (k in sources) {
  col= if(k=="Matheus") {col="gray30"} else {col='gray60'}
  lines(B.Bmsy$b[B.Bmsy$source==k],F.Fmsy$f[F.Fmsy$source==k],lwd=2,col=col)
  points(B.Bmsy$b[B.Bmsy$source==k][1],F.Fmsy$f[F.Fmsy$source==k][1],col=col,pch=19,bg='white',cex=1.5)
  points(B.Bmsy$b[B.Bmsy$source==k][nyr],F.Fmsy$f[F.Fmsy$source==k][nyr],col=col,pch=17,bg='white',cex=1.5)
}
...
legend('topright',
       ...
       pt.bg=c("gray",rep("gray40",2),"gray30","gray60","red","yellow","orange","green"),
       col=c("gray",rep("gray40",2),"gray30","gray60",rep(1,4)),lwd=c(NA,rep(2.5,4),rep(1.1,4)),
...
if(length(sources)==1){
  legend('topright',
         ...
         pt.bg=c("gray",rep("gray40",2),"gray30","red","yellow","orange","green"),
         col=c("gray",rep("gray40",2),"gray30",rep(1,4)),lwd=c(NA,rep(2.5,3),rep(1.1,4)),
...
write.table(kobeout, file = "kobeout.csv", 
            dec=".",sep = ",", row.names = FALSE) 

#write kobe out with f/fmsy and b/bmsy series
write.table(bioout, file = "bioout.csv", 
            dec=".",sep = ",", row.names = FALSE) 
